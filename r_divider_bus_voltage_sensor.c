...
static uint32_t voltage_startup_delay_counter = 0;
#define VOLTAGE_STARTUP_IGNORE_COUNT  1000  // or time-based delay
...
uint16_t RVBS_CheckFaultState(RDivider_Handle_t *pHandle)
{
  ...
  if(voltage_startup_delay_counter < VOLTAGE_STARTUP_IGNORE_COUNT)
    {
	  voltage_startup_delay_counter++;
